...
def sendSignupEmail(email,token, prod):
	message = f'Subject:Account creation link\n\nThe link to set up EmAuth account is https://{deployed_domain}/email_signup_user?token='+token
	sendmail(email, message)

def sendNewReqMail(email, requester, prod):
	message = "Subject:Authentication Request\n\nNew authentication request on EmAuth. Details:\n"+requester+"\n\nCheck the app for more details."
	sendmail(email, message)
...
def adminPromoteMail(email, prod):
	message = f"Subject:Promote to Admin\n\nYou have been promoted to admin in EmAuth. To go to admin dashboard, use https://{deployed_domain}/admin"
	sendmail(email, message)
